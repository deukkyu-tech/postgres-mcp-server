...
async def main():
    ...
    parser.add_argument(
        "--user-id", default="test_user", help="User ID for server context (optional)"
    )
    parser.add_argument(
        "--conn-id",
        help="Connection ID for server context (optional)",
    )
    ...
    if args.conn_id:
        server_instance = server_creator(user_id=args.user_id, conn_id=args.conn_id)
    else:
        server_instance = server_creator(user_id=args.user_id)

    logger.info(
        f"Starting local stdio server for server: {args.server} with user: {args.user_id or 'None'} with conn_id: {args.conn_id or 'None'}"
    )
