...
def load_test_module(server_name):
    ...
    test_file = project_root / "tests" / "servers" / server_name / "test.py"
...
async def run_server_tools(server_name,conn_id=None):
    ...
    logger.info(f"Running tests for server: {server_name}, args:{args}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conn-id", help="Optional connection ID")
    args = parser.parse_args()
    asyncio.run(run_server_tools(args.server, conn_id=args.conn_id))